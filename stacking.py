...
#possibilidade = [["svc","rf","rf"],["svc","rf","knn"],["svc","rf","svc"],["svc","rf","rocchio"],["svc","knn","rf"],["svc","knn","knn"],["svc","knn","svc"],["svc","knn","rocchio"],["svc","svc","rf"],["svc","svc","knn"],["svc","svc","svc"],["svc","svc","rocchio"]]
possibilidade = [["rf","rf","svc"]]
#possibilidade = [['svc', 'rf', 'knn']]
...
#            for i in [0.001,1,10,100,1000,2000,2500,3000]: #teste de hiperparametro
            y_prob_predito = supportVectorMachine.svc(X_train, X_test, y_train, y_test,"prob",2500)
            resultadoOHE = pd.concat([resultadoOHE,pd.DataFrame(y_prob_predito)],axis=0)
        if(algoritmo2 =="rf"):
...
#            for i in [1,2,3,5,6,7,8,9,10]: #teste de hiperparametro
            y_prob_predito_text = knn.knn(X_train_text, X_test_text, y_train, y_test,"prob",6)
            resultadoTFIDF = pd.concat([resultadoTFIDF,pd.DataFrame(y_prob_predito_text)],axis=0)
        else:
#            for i in [0.001,1,10,100,1000,2000,2500,3000]: #teste de hiperparametro
