...
label, index_label_x_empenhos = tratar_label.label_elemento(label['Natureza Despesa (Cod)(EOF)'], 3)
...
dados = pickles.carregarPickle("data")
tfidf = pickles.carregarPickle("tfidf")
label = pickles.carregarPickle("label")
dados = sparse.hstack((csr_matrix(dados),csr_matrix(tfidf) ))
del tfidf
...
dados_test, label_test = tratarDados(dados_test.copy(),'OHE')
dados_test = sparse.hstack((csr_matrix(dados_test),csr_matrix(tfidf_teste) ))
del tfidf_teste
...
# =============================================================================
    if(TREINAR_MODELOS):
        ...
        print("RF")
        randomforest.randomForest(X_train, X_test, y_train, y_test,"Randon Forest Fold "+ str(fold_number), 100)
        print("SGD")
...
#        except:
#            modelo = joblib.load("pickles/oraculo/"+modelos[i]+ str(fold_number)+".joblib")
...
#    # =============================================================================
#    avaliacao = "f1_macro"
#    espalhamento = 3 # espalhamento se refere a quantidade de valores criados na vizinhanca do melhor valor para o fine search
#    print("Rodando o algoritmo do oraculo")
#    # Retirando classes com menos de 3 documentos para o cross validation
#    label_hiperparametros, index_label_x_empenhos = tratar_label.label_elemento(algoritmos[0], 3)
#    dados_hiperparametros= dados.drop(index_label_x_empenhos, axis = 0)
#    dados_hiperparametros.reset_index(drop = True, inplace = True)
#    if(Algoritmo_oraculo == 1):
#        melhor_alg = "Randon Forest -fixed"
#        modelo = RandomForestClassifier(n_jobs=-1, random_state=10)
#        hiperparametros = {'n_estimators':[100,300,500,700] }
#        melhor_n_estimators = refinamento_hiperparametros(csr_matrix(dados_hiperparametros), label_hiperparametros, modelo, hiperparametros, espalhamento, avaliacao)["n_estimators"]
#        modelo = RandomForestClassifier(n_jobs=-1, n_estimators = melhor_n_estimators, random_state=10)
#        modelo.fit(csr_matrix(dados),algoritmos.values.ravel())
#        y_alg = modelo.predict(csr_matrix(dados_test))
...
#            modelo = RandomForestClassifier(n_jobs=-1, random_state=10)
#            hiperparametros = {'n_estimators':[100,300,500,700] }
#            melhor_hiperparametro = refinamento_hiperparametros(csr_matrix(dados_hiperparametros), label_hiperparametros, modelo, hiperparametros, espalhamento, avaliacao)["n_estimators"]
#            modelo = RandomForestClassifier(n_jobs=-1,n_estimators=melhor_hiperparametro,random_state=10)
#        elif("Knn" in melhor_alg):
#            modelo = KNeighborsClassifier(n_jobs = -1)
#            hiperparametros = {'n_neighbors':[1,3,5,7] }
#            melhor_hiperparametro = refinamento_hiperparametros(csr_matrix(dados_hiperparametros), label_hiperparametros, modelo, hiperparametros, espalhamento, avaliacao)["n_neighbors"]
#            modelo = KNeighborsClassifier(n_neighbors = melhor_hiperparametro,n_jobs=-1)
#        elif("NB" in melhor_alg):
#            modelo = GaussianNB()
#        elif("SGD" in melhor_alg):
#            modelo = SGDClassifier(loss = "log", random_state = 10)
#            hiperparametros = {'max_iter':[100,300,500,700] }
#            melhor_hiperparametro = refinamento_hiperparametros(csr_matrix(dados_hiperparametros), label_hiperparametros, modelo, hiperparametros, espalhamento, avaliacao)["max_iter"]
#            modelo = SGDClassifier(max_iter = melhor_hiperparametro, loss = "log", random_state = 10)
#        elif("SVC 0.1" in melhor_alg):
#            modelo = SVC(kernel="linear",C = 0.1,random_state = 10)
#        elif("SVC 10" in melhor_alg):
#            modelo = SVC(kernel="linear",C = 10,random_state = 10)   
#        
...
#            y_alg = bst.predict(dtest)
#            y_alg = pd.DataFrame(y_alg).astype("int16")
#            y_alg = pd.DataFrame(encoder.inverse_transform(y_alg.values.ravel()))
#        elif("NB" in melhor_alg):
#            modelo.fit(dados, algoritmos.values.ravel())
#            y_alg = modelo.predict(dados_test)
#            y_alg = pd.DataFrame(encoder.inverse_transform(y_alg))
#        else:
#            modelo.fit(csr_matrix(dados), algoritmos.values.ravel())
#            y_alg = modelo.predict(csr_matrix(dados_test))
...
#                    model = joblib.load("pickles/oraculo/"+modelos[modelo]+str(fold)+".joblib")
#                else:
...
#                model = joblib.load("pickles/oraculo/"+y_alg.value_counts().index[k][0]+".joblib")
#            else:
